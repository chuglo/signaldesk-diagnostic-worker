...
COPY pyproject.toml README.md uv.lock /build/signaldesk-diagnostic-worker/
COPY src /build/signaldesk-diagnostic-worker/src
RUN python -m pip install uv==0.11.31 \
    && cd /build/signaldesk-diagnostic-worker \
...
USER 10001:10001
CMD ["signaldesk-diagnostic-worker"]
